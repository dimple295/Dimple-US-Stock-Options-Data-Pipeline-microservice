...
from src.preprocess import fetch_data_from_api, preprocess_data
...
def train_model(data_api_url, is_fine_tune=False):
    try:
        ...
        num_features = len(SYMBOLS) * 5  # 5 features per stock
        ...
        if is_fine_tune:
            ...
            start_date = end_date - timedelta(days=7)  # Use last 7 days for fine-tuning
            interval = "1min"
            source = "influxdb"
        ...
        df = fetch_data_from_api(data_api_url, start_date, end_date, interval=interval, source=source)
        X_train, y_train, X_val, y_val, X_test, y_test, indices_test, scaler, feature_columns = preprocess_data(
            df, SYMBOLS, seq_length=120, forecast_horizon=240, is_1min=(interval == "1min")
        )
...
if __name__ == "__main__":
    data_api_url = os.getenv("DATA_API_URL", "http://data-api-service:8006/api/realtime-data/")
